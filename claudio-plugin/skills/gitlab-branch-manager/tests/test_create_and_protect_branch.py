...
# Mock jq implemented in Python — handles the jq patterns used by the script.
# Placed on PATH alongside mock glab so tests need no real jq binary.
...
def main():
    ...
    data = None if no_input else json.loads(sys.stdin.read())
    result = evaluate(filter_expr, data, variables)
    output(result, raw_output)

def evaluate(expr, data, variables):
    expr = expr.strip()

    # '. + {($k): $v}' — merge variable-keyed pair into object
    if expr == ". + {($k): $v}":
        result = dict(data)
        result[variables["k"]] = variables["v"]
        return result

    # '.[].field' — extract field from all array elements
    m = re.match(r"^\.\[\]\.([\w]+)$", expr)
    if m:
        field = m.group(1)
        return "\n".join(str(item.get(field, "")) for item in data)

    # select + length check (resolve_repo_by_name)
    if "select(.path == $name)" in expr and "length" in expr:
        name = variables["name"]
        matches = [item for item in data if item.get("path") == name]
        if ".[].path_with_namespace" in expr:
            return "\n".join(item.get("path_with_namespace", "") for item in matches)
        if len(matches) == 1:
            return matches[0].get("path_with_namespace", "")
        elif len(matches) == 0:
            return "NO_MATCH"
        else:
            return "MULTIPLE"

    # select + extract field (MULTIPLE candidates list)
    if "select(.path == $name)" in expr and "path_with_namespace" in expr:
        name = variables["name"]
        matches = [item for item in data if item.get("path") == name]
        return "\n".join(item.get("path_with_namespace", "") for item in matches)

    # extract_protection_value — nested access levels
    if "access_level" in expr and "plural" in variables:
        plural = variables["plural"]
        key = variables["key"]
        arr = data.get(plural)
        if isinstance(arr, list) and len(arr) > 0:
            return str(arr[0].get("access_level", ""))
        elif key in data:
            return str(data[key])
        return ""

    # extract_protection_value — simple field access with has()
    if "has($key)" in expr and "tostring" in expr:
        key = variables["key"]
        if key in data:
            val = data[key]
    ...
    # Object construction template '{key: $var, ...}'
    if expr.lstrip().startswith("{"):
        return build_object(expr, variables)

    return data

def build_object(template, variables):
    # Parse '{key: $var, ...}' and '{key: literal, ...}' including multiline
    result = {}
    for m in re.finditer(r"(\w+)\s*:\s*(\$\w+|true|false|null|\d+)", template):
        key, val = m.group(1), m.group(2)
        if val.startswith("$"):
            var = val[1:]
            if var in variables:
                result[key] = variables[var]
        elif val == "true":
            result[key] = True
        elif val == "false":
            result[key] = False
...
"""
    ...
    # Also create mock jq in the same directory
    mock_jq = tmp_path / "jq"
    mock_jq.write_text(MOCK_JQ_SCRIPT)
    mock_jq.chmod(mock_jq.stat().st_mode | stat.S_IEXEC)

    return str(tmp_path), str(log_file)


def _run_script(mock_dir, args, env_extras=None):
    """Run create_and_protect_branch.sh with mock glab and jq on PATH."""
    env = os.environ.copy()
